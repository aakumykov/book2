...
class SimpleTidy
  ...
  OPTIONS_WITHOUT_VALUES = [ :indent, :utf8, :ashtml, :asxhtml, :asxml, :numeric ]
...
when 1
	input_file = ARGV[0]
	output_file = 'out.xhtml'
when 2
	input_file = ARGV[0]
	output_file = ARGV[1]
else
	puts "Usage #{__FILE__} <input.html>"
	exit 1
end
...
File.open(output_file,'w') { |file|
       file.write( 
		  st.clean(File.read(input_file))
	 )
}


system "ls -lh #{output_file}"
